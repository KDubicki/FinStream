...
> Stays **Proposed** until plan 0001 research item R2 confirms the API details below. Then it gets accepted, or superseded if R2 changes the decision.
...
|---|---|---|---|
| APScheduler 3.11.x (chosen) | Mature; cron + interval triggers; timezones; coalesce/misfire/max_instances | In-memory store loses schedule state on restart; single instance only | **Proposed** |
...
- **Day-of-week numbering:** APScheduler's cron day-of-week numbering may differ from classic cron (UNVERIFIED until R2), so day names are mandatory in `DAILY_CRON`.
