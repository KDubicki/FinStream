...
- **Call:** one call per symbol through yfinance (`Ticker.history` or `download`, decided in R1) with auto-adjust disabled, so both `close` and `adj_close` are stored raw.
- **Error classification** (confirmed in R1):

  | Error | Raised as | Retried? |
  |---|---|---|
  | network / timeout / 5xx | `TransientSourceError` | yes |
  | rate limit | `RateLimitedError` | yes |
  | invalid or delisted symbol | `PermanentSourceError` | no |
  | empty frame | returns `[]` | not an error |
...
- **`init_schema(engine, timescaledb_enabled)`:** idempotent DDL in one transaction.
...
services:
  db:
    image: timescale/timescaledb:<R3-pinned-tag>
    environment:
...
A PyPI snapshot from 2026-09-15 gives the starting point for R4. These are **not final pins**:
...
Each milestone is committed right away as a Conventional Commit, after its gate is green.
