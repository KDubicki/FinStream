...
- **Why the primary key includes `ts`:** TimescaleDB requires every unique constraint on a hypertable to include the partitioning column.
...
- **`migrate_data => TRUE`:** lets a database that started with `TIMESCALEDB_ENABLED=false` be converted later. On existing hypertables `if_not_exists` makes this a no-op. The exact syntax gets verified in plan 0001 R3.
